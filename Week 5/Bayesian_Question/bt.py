...
def naive_bayes_classify(info, sentence):
    ...
    dict=info.word_counts
  
    P={}
    
    
    total_senti_count=0
    for i in info.sentiment_counts:
        total_senti_count+=i
    ...
    for c in range(CLASSES):
        ...
        for word in tokenize(sentence):
            ...
            if word in dict[c]:
                ...
                prob*=(dict[c][word]/info.total_words[c])
            
            else:
                
                prob*= OUT_OF_VOCAB_PROB
        
        P_of_senti= (info.sentiment_counts[c]/total_senti_count)
        prob= math.log(prob*P_of_senti) #Probability of that class.
        P[c]=prob
        
    Keymax = max(P, key=P.get) 
    
    return Keymax, P[Keymax]  # best class, log probability
...
def markov_model_classify(info, sentence):
    ...
    dict_2 = info.bigram_counts
    dict_1= info.word_counts
    ...
    for c in range(CLASSES):
        ...
        prob=1
        first_term=1
        #Performing Naive Bayes for index 1.
        
        if first_term==1:
            if words[0] in info.word_counts[c]:
                    
                prob*=(dict_1[c][words[0]]/info.total_words[c])
        ...
        while current < len(words):    
            ...
            if ((words[previous],words[current]) in dict_2[c]):
                
                prob*=(info.bigram_counts[c][(words[previous],words[current])] / info.bigram_denoms[c][words[previous]])
