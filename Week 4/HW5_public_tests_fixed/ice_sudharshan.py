...
def Q_max(moves,V_copy,i, j,problem):
...
def mdp_solve(problem, iterations):
    ...
    V = [[0 if problem.map[i][j] == '-' else GOLD_REWARD if problem.map[i][j] == 'G' else PIT_REWARD for j in
          range(len(problem.map[0]))] for i in range(len(problem.map))]
    while iterations > 0:
        ...
        for i in range(len(problem.map)):
            for j in range(len(problem.map[0])):
                if problem.map[i][j] == 'G' or problem.map[i][j] == 'P':
                    continue
                key, value = Q_max(legalMoves(i,j),V_copy, i,j,problem)
                V[i][j] = value
                problem.map[i][j] = key
        iterations -= 1
