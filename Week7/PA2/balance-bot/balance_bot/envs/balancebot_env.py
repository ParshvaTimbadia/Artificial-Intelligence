...
class BalancebotEnv(gym.Env):
    ...
    def __init__(self, render=True):
        self._observation = []
        self.action_space = spaces.Discrete(7) #Decreasing the actionspace.
    ...
    def _reset(self):
        ...
        p.resetSimulation()
        p.setGravity(0,0,-6) # m/s^2 #Changed the gravity
        ...
        #This code works on proper installation. 
        
        # self.wallId = p.loadURDF(os.path.join(path,"obstacles.xml"),[0,0,1],cubeStartOrientation) 
        #I am getting pybullet error in my latop but works fine in other so just try running it. 
    ...
    def _assign_throttle(self, action):
        dv=random.randrange(10)#Change
        dv=dv/100
        deltav = [-10.*dv, -2.*dv, -0.1*dv, 0, 0.1*dv, 2.*dv, 10.*dv][action] #Removed two values, since 
        #Actionspaces is reduced. Hence, faster learning 
